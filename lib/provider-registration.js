...
async function registerWithProvider(srf, provider, logger) {
  ...
  // Normalize transport from backend enum to SIP value
  // Backend sends: "SIP_TRANSPORT_UDP" → Convert to "udp"
  let normalizedTransport = 'udp'; // default
  if (transport) {
    const transportStr = String(transport).toLowerCase();
    if (transportStr.includes('tcp')) {
      normalizedTransport = 'tcp';
    } else if (transportStr.includes('tls')) {
      normalizedTransport = 'tls';
    } else {
      normalizedTransport = 'udp';
    }
  }
  ...
  const contactUri = `sip:${publicIp}:${publicSipPort};transport=${normalizedTransport}`;
  ...
  logger.info(
    { providerName: name, registrar: host, username, contactUri, publicIp, port: publicSipPort, transport: normalizedTransport },
