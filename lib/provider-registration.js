...
async function registerWithProvider(srf, provider, logger) {
  ...
  const contactUri = `sip:${username}@${publicIp}:${process.env.PUBLIC_SIP_PORT || 5060};transport=${transportLower}`;
  ...
  logger.info(
    { providerName: name, registrar: host, username },
...
async function updateProviderRegistrationStatus(
  ...
  try {
    ...
    logger.error(
      { error: err.message, providerId, status },
