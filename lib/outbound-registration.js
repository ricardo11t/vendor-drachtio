...
async function registerTrunk(srf, trunk, logger) {
  ...
  try {
    ...
    // Send REGISTER via Drachtio using srf.request (createUAC is for dialogs only)
      ...
      srf.request(registrarUri, opts, (err, req) => {
        ...
        req.on('response', (res) => {
          if (res.status === 200 || res.status === 401) {
            // 200 = success, 401 = challenge (handled by srf if auth provided, but we log success here)
            // Note: srf handles the 401 challenge automatically if opts.auth is present
            // We might receive a 200 OK after the challenge
            ...
            if (res.status === 200) {
              ...
              // Clear any existing interval if we want to be robust, but for now just set new
              // In a real app we'd track intervals to clear them
              setTimeout(() => {
                registerTrunk(srf, trunk, logger).catch((err) => {
                  logger.error(
                    { trunkName: name, error: err.message },
             ...
             // If 401, srf will resend with auth. We wait for the next response (200 or 403)
             // Actually, srf.request callback is called once. 
             // If srf handles auth, does it call callback again? 
             // Usually srf.request handles the challenge internally and emits 'response' for the FINAL response?
             // Or we need to handle the 401 ourselves?
             // Documentation says "srf will handle authentication challenges".
             // So we should expect the final response here.
          } else {
            logger.error(
