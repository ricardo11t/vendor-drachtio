...
const rtpOffer = async (payload) => {
  return new Promise((resolve, reject) => {
    ...
    engine.offer(RTPENGINE_PORT, RTPENGINE_HOST, payload, (err, res) => {
      if (err) {
        console.error('RtpEngine Offer Error Details:', err);
        return reject(err);
      }
      resolve(res);
    });
  ...
  async connect() {
    try {
      ...
      try {
        ...
        this.logger.info({ fullDestination: JSON.stringify(destination) }, '📥 Full destination response from API');
        
        if (destination && destination.destinationUri) {
          this.destinationUri = destination.destinationUri;
          this.roomName = destination.roomName;
          this.logger.info({ destinationUri: this.destinationUri, roomName: this.roomName }, '✅ Destination URI from API');
        } else {
          this.logger.warn({ fallback: this.destinationUri }, '⚠️ API returned empty destination, using fallback');
        }
      } catch (err) {
        this.logger.warn(
      ...
      // Validate destinationUri is set
      if (!this.destinationUri || typeof this.destinationUri !== 'string') {
        this.logger.error('❌ CRITICAL: destinationUri is not a valid string');
        return this._sendErrorResponse(500, 'Invalid destination configuration');
      }
      ...
      // In drachtio-srf, req.body is the message body after headers (the SDP)
      const incomingSdp = this.req.body;
      ...
      if (!offerResponse || offerResponse.result !== 'ok') {
        this.logger.error(
          { offerResponse: JSON.stringify(offerResponse).substring(0, 1000), payload: JSON.stringify(offerPayload).substring(0, 500) },
          '❌ RtpEngine Offer failed'
        );
        throw new Error(
      ...
      const b2buaHeaders = {
        ...
        'Contact': `<sip:${publicIp}:${publicSipPort || 5060};transport=${sipTransport || 'udp'}>`
      };

      // Add room name header if available
      if (this.roomName) {
        b2buaHeaders['X-LiveKit-Room-Name'] = this.roomName;
      }

      this.logger.info(
        { 
          headers: b2buaHeaders, 
          destinationUri: this.destinationUri,
          sdpLength: offerResponse.sdp?.length,
          callId: this.callId
        },
        '🔍 B2BUA REQUEST DETAILS - Headers and SDP being sent to LiveKit'
      );
      ...
      const { uas, uac } = await this.srf.createB2BUA(this.req, this.res, this.destinationUri, {
        ...
        localSdpA: async (sdpFromLiveKit, res) => {
          this.logger.info(
            { 
              sdpLength: sdpFromLiveKit?.length || 0, 
              statusCode: res.statusCode,
              statusMessage: res.statusMessage,
              callId: this.callId
            },
            '✅ Received response from LiveKit - Processing SDP'
          );

          // Log full SDP preview
          if (sdpFromLiveKit) {
            this.logger.debug(
              { 
                sdpPreview: sdpFromLiveKit.substring(0, 150),
                callId: this.callId
              },
              '📋 LiveKit SDP preview'
            );
          ...
          this.logger.info(
            { sdpLength: answerResponse.sdp?.length || 0, callId: this.callId },
      ...
      // ========================================================
      // STEP 6: MONITOR CALL STATE & CLEANUP ON DESTROY
      // ========================================================
      
      // Log when UAS (incoming from WavOIP) connects
      uas.on('connect', () => {
        this.logger.info({ callId: this.callId }, '📞 UAS (WavOIP side) connected');
      });

      // Log when UAC (outgoing to LiveKit) connects
      uac.on('connect', () => {
        this.logger.info({ callId: this.callId }, '📞 UAC (LiveKit side) connected');
      });

      // Log when UAS is destroyed
      uas.on('destroy', () => {
        this.logger.info({ callId: this.callId, reason: uas.logger?.getReason?.() }, '📤 UAS destroyed (WavOIP side ended)');
      });

      // Log when UAC is destroyed
      uac.on('destroy', () => {
        this.logger.info({ callId: this.callId, reason: uac.logger?.getReason?.() }, '📤 UAC destroyed (LiveKit side ended)');
      });

      // Log errors on both sides
      uas.on('error', (err) => {
        this.logger.error({ callId: this.callId, error: err.message }, '❌ UAS error');
      });

      uac.on('error', (err) => {
        this.logger.error({ callId: this.callId, error: err.message }, '❌ UAC error');
      });

      const cleanup = async () => {
        this.logger.info('🧹 Cleaning up RtpEngine and B2BUA');
        try {
